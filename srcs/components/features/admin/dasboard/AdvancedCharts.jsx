...
export const RevenueAreaChart = ({ revenueData, profitData }) => {
  ...
  return (
    <div className="h-80 w-full p-4">
      <h3 className="text-base font-semibold text-gray-700 mb-4">Doanh thu & Lợi nhuận trong năm</h3>
        ...
        >
          <defs>
            <linearGradient id="colorRevenue" x1="0" y1="0" x2="0" y2="1">
              <stop offset="5%" stopColor={REVENUE_COLORS.revenue.start} stopOpacity={0.8} />
              <stop offset="95%" stopColor={REVENUE_COLORS.revenue.end} stopOpacity={0.2} />
            </linearGradient>
            <linearGradient id="colorProfit" x1="0" y1="0" x2="0" y2="1">
              <stop offset="5%" stopColor={REVENUE_COLORS.profit.start} stopOpacity={0.8} />
              <stop offset="95%" stopColor={REVENUE_COLORS.profit.end} stopOpacity={0.2} />
            </linearGradient>
          </defs>
          <XAxis dataKey="name" tick={{ fontSize: 10 }} />
          <YAxis tickFormatter={(value) => value >= 1000000 ? `${(value / 1000000).toFixed(0)}tr` : `${(value / 1000).toFixed(0)}k`} />
...
export const TargetComparisonChart = ({ revenueData, profitData }) => {
  ...
  return (
    <div className="h-80 w-full p-4">
      <h3 className="text-base font-semibold text-gray-700 mb-4">So sánh doanh thu với mục tiêu</h3>
      <ResponsiveContainer width="100%" height="100%">
        <ComposedChart
          data={data}
          margin={{ top: 20, right: 40, left: 10, bottom: 20 }}
        >
          ...
          <defs>
            <linearGradient id="barGradient" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor={REVENUE_COLORS.revenue.start} stopOpacity={0.8} />
              <stop offset="100%" stopColor={REVENUE_COLORS.revenue.end} stopOpacity={0.8} />
            </linearGradient>
          </defs>
          <Bar 
            dataKey="doanhThu" 
...
export const QuarterlyRevenueChart = ({ revenueData, profitData }) => {
  ...
  return (
    <div className="h-80 w-full p-4">
      <h3 className="text-base font-semibold text-gray-700 mb-4">Phân tích doanh thu theo quý</h3>
      <ResponsiveContainer width="100%" height="100%">
        <BarChart
          data={data}
          margin={{ top: 20, right: 40, left: 10, bottom: 20 }}
...
export const ProductCategoryPieChart = () => {
  ...
  return (
    <div className="h-80 w-full p-4">
      <h3 className="text-base font-semibold text-gray-700 mb-4">Danh mục sản phẩm bán chạy</h3>
      <ResponsiveContainer width="100%" height="100%">
...
export const BusinessMetricsRadarChart = () => {
  ...
  return (
    <div className="h-80 w-full p-4">
      <h3 className="text-base font-semibold text-gray-700 mb-4">Chỉ số kinh doanh (% so với mục tiêu)</h3>
      <ResponsiveContainer width="100%" height="100%">
        <RadarChart cx="50%" cy="50%" outerRadius="75%" data={data} margin={{ top: 20, right: 40, left: 10, bottom: 20 }}>
          <PolarGrid />
          <PolarAngleAxis dataKey="subject" />
          <PolarRadiusAxis angle={30} domain={[0, 100]} />
          <Radar name="Hiệu suất" dataKey="A" stroke="#8884d8" fill="#8884d8" fillOpacity={0.6} />
          <Legend wrapperStyle={{ paddingTop: 10 }} />
          <Tooltip formatter={(value) => `${value}%`} />
        </RadarChart>
      </ResponsiveContainer>
...
export const PriceVsRatingScatterChart = () => {
  // Dữ liệu mẫu
  const sampleData = [
    { price: 12000000, rating: 4.8, sales: 250, name: 'Laptop Gaming ASUS TUF' },
    ...
    { price: 17500000, rating: 4.6, sales: 210, name: 'Laptop Gaming Predator' },
  ...
  return (
    <div className="h-80 w-full p-4">
      <h3 className="text-base font-semibold text-gray-700 mb-4">Phân tích giá và đánh giá sản phẩm</h3>
      <ResponsiveContainer width="100%" height="100%">
        <ScatterChart
        ...
        >
          ...
          <XAxis 
            ...
            label={{ value: 'Giá (triệu VNĐ)', position: 'insideBottomRight', offset: 0 }}
          />
          <YAxis 
            type="number" 
            dataKey="rating" 
            name="Đánh giá" 
            domain={[3.5, 5]}
            label={{ value: 'Đánh giá (sao)', angle: -90, position: 'insideLeft' }}
          />
          <Tooltip 
              ...
              if (active && payload && payload.length) {
                ...
                return (
                  <div className="bg-white p-2 border border-gray-200 shadow-sm rounded">
                    ...
                    <p className="text-xs">Đã bán: {formatNumber(data.sales)}</p>
                  </div>
                );
              }
              return null;
...
export const SalesByProductTreemap = () => {
  ...
  return (
    <div className="h-80 w-full p-4">
      <h3 className="text-base font-semibold text-gray-700 mb-4">Cơ cấu doanh số theo nhóm sản phẩm</h3>
      <ResponsiveContainer width="100%" height="100%">
        <Treemap
          data={data}
          dataKey="size"
            ...
            content={({ active, payload }) => {
              if (active && payload && payload.length) {
                ...
                return (
                  <div className="bg-white p-2 border border-gray-200 shadow-sm rounded">
                    <p className="font-medium text-sm">{data.name}</p>
                    <p className="text-xs">Doanh số: {formatNumber(data.size || 0)} sản phẩm</p>
                  </div>
                );
              }
              return null;
