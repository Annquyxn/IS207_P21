      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Tổng cộng {statusCounts.total || 0} đơn hàng | Doanh thu {formatCurrency(orderStats.revenue || 0)}
          </p>
        </div>
                ...
                filteredOrders.map((order) => (
                  ...
                  >
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="font-medium text-gray-900">ORD-{order.id.toString().substring(0, 8)}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {order.product_info ? order.product_info.quantity : 1} sản phẩm
                    </td>
            ...
            >
              <div className="p-6 border-b border-gray-100">
                <div className="flex justify-between items-start">
                  <div>
                    <h2 className="text-xl font-bold text-gray-800">Chi tiết đơn hàng #{selectedOrder.id.toString().substring(0, 8)}</h2>
                    <p className="text-sm text-gray-500 mt-1">Ngày đặt: {new Date(selectedOrder.order_date).toLocaleDateString('vi-VN')}</p>
                  </div>
              ...
              <div className="p-6 space-y-6">
                <div>
                  <h3 className="text-base font-medium text-gray-800 mb-2">Thông tin khách hàng</h3>
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <p><span className="font-medium">Họ tên:</span> {selectedOrder.customer_name}</p>
                    <p><span className="font-medium">Giới tính:</span> {selectedOrder.gender === 'male' ? 'Nam' : selectedOrder.gender === 'female' ? 'Nữ' : 'Khác'}</p>
                    ...
                    {selectedOrder.address?.note && (
                      <p><span className="font-medium">Ghi chú:</span> {selectedOrder.address.note}</p>
                    )}
                  </div>
                </div>
                
                <div>
                  <h3 className="text-base font-medium text-gray-800 mb-2">Sản phẩm</h3>
                    ...
                    {selectedOrder.product_info && (
                      <div className="flex justify-between items-center border-b border-gray-200 pb-2">
                        <div className="flex items-center">
                          ...
                          <div>
                            <p className="font-medium">{selectedOrder.product_info.title}</p>
                            <p className="text-sm text-gray-500">Số lượng: {selectedOrder.product_info.quantity || 1}</p>
                          </div>
                        </div>
                        <p className="font-medium">{formatCurrency(selectedOrder.product_info.price)}</p>
                      </div>
                    )}
                    
                    <div className="flex justify-between items-center pt-2">
                      <p className="text-sm">Tổng tiền sản phẩm:</p>
                      <p className="font-medium">{formatCurrency(selectedOrder.product_price)}</p>
                    </div>
                    
                    <div className="flex justify-between items-center">
                      <p className="text-sm">Phí vận chuyển:</p>
                      <p className="font-medium">{formatCurrency(selectedOrder.shipping_fee)}</p>
                    </div>
                    
                    {selectedOrder.discount > 0 && (
                      <div className="flex justify-between items-center">
                        <p className="text-sm">Giảm giá:</p>
                        <p className="font-medium text-red-600">-{formatCurrency(selectedOrder.discount)}</p>
                      </div>
                    )}
                    
                    <div className="flex justify-between items-center pt-2 border-t border-gray-200">
                      <p className="font-bold">Tổng cộng:</p>
                      <p className="font-bold text-red-600">{formatCurrency(selectedOrder.total)}</p>
                    </div>
                  </div>
                </div>
                
                <div>
                  <h3 className="text-base font-medium text-gray-800 mb-2">Thông tin vận chuyển</h3>
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <p><span className="font-medium">Phương thức vận chuyển:</span> {selectedOrder.shipping_method === 'standard' ? 'Tiêu chuẩn' : 'Nhanh'}</p>
                    <p><span className="font-medium">Phương thức thanh toán:</span> {selectedOrder.payment_method === 'cod' ? 'Thanh toán khi nhận hàng' : 'Chuyển khoản ngân hàng'}</p>
                  </div>
                </div>
