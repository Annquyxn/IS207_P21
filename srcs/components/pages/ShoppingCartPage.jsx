...
const ShoppingCartPage = () => {
  ...
  const handleRemoveItem = (itemId) => {
    removeFromCart(itemId, ({ removedProduct }) => {
      toast.success(`Đã xóa ${removedProduct.name || 'sản phẩm'} khỏi giỏ hàng`);
    });
  };
  ...
  const prepareProductForOrder = (item) => {
    return {
      ...
      brand: item.brand,
      image: item.image,
      originalPrice: item.originalPrice ? formatPrice(item.originalPrice) : formatPrice(0),
  ...
  const handleCheckout = () => {
    ...
    const singleProduct = cart.length === 1 
            ...
            <div className="mt-8 border-t pt-6">
              <div className="bg-gray-50 rounded-xl p-6">
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  >
                    <span>{cart.length === 1 ? 'Đặt hàng ngay' : 'Tiến hành thanh toán'}</span>
                    <FiArrowRight />
                  </motion.button>
