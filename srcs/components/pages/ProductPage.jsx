...
function ProductPage() {
  ...
  // Sử dụng useMemo để cache danh sách sản phẩm
  ...
  const processedProducts = useMemo(() => {
    ...
    if (selectedBrand !== "all") {
      filtered = filtered.filter((product) =>
        product.title.toLowerCase().includes(selectedBrand)
      );
    }

    // Sắp xếp
    return [...filtered].sort((a, b) => {
      const getPrice = (str) => parseInt(str.replace(/\D/g, ""));
      const priceA = getPrice(a.salePrice);
      const priceB = getPrice(b.salePrice);

      if (sortBy === "price-asc") return priceA - priceB;
      if (sortBy === "price-desc") return priceB - priceA;
      return 0;
    });
  }, [allProducts, selectedBrand, sortBy]);

  // Load products ban đầu
  useEffect(() => {
    setLoading(true);
